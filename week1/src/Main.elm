...
last1 = List.reverse >> List.head
-- [1, 2, 3] => [3, 2, 1] => Just 3

last2 : List a -> Maybe a
last2 = List.foldl (Just >> always) Nothing
...
butLast1 = List.reverse >> List.tail >> Maybe.andThen List.head
...
length1 : List a -> Int
length1 = List.foldl (\_ acc -> acc + 1) 0

length2 : List a -> Int
length2 = List.foldl (increment |> always) 0
    ...
    let
        ...
        iterator lhs rhs =
            case ( lhs, rhs ) of
                ...
                ( x :: xs, y :: ys ) ->
                    if x == y then iterator xs ys else False
...
dropEvery number string =
    let
        iterator char ( pos, str ) =
            ( pos + 1
            , if remainderBy number pos == 0 then str else str ++ String.fromChar char
            )
    in
...
clap1 : String -> String
clap1 = String.words >> String.join " 👏 "
        ...
        iterator headTail buf list =
            case headTail of
                Nothing ->
                    ...
                    if String.isEmpty buf then list else list ++ [buf]

                Just ( char, str ) ->
                    if List.member char separators then
                        -- add buf to list if not empty
                        iterator (String.uncons str) "" (if String.isEmpty buf then list else list ++ [buf])
                    else
                        -- add char to buf
